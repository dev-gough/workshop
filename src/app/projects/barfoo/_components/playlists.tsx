...
import { ChevronLeft, Play, Plus, Shuffle, Trash2 } from 'lucide-react';
import { motion } from 'motion/react';
import { Button } from '@/components/ui/button';
import { cleanSongDisplay } from '@/lib/songUtils';
import type { Playlist, PlaylistDetail } from './shared';

export function PlaylistsView({ playlists, active, onOpen, onBack, onDelete, onPlay, onRemoveSong, onNew }: {
  playlists: Playlist[];
      ...
      {!active ? (
        ...
        <>
          <div className="flex items-center gap-3">
            ...
            <h2 className="bar-serif flex-1 truncate text-lg font-semibold">{active.name}</h2>
            <Button size="sm" onClick={() => onPlay(active.songs)} className="h-8 text-xs">
              <Play className="mr-1 h-3.5 w-3.5" />Play
            </Button>
            <Button size="sm" variant="outline" onClick={() => onPlay(active.songs, true)} className="h-8 text-xs">
