...
export function ChallengeHoverCard({ node, rect }: { node: ChallengeNode; rect: DOMRect }) {
  ...
  return createPortal(
    // `lol-theme` has to be repeated here: the card is portaled to document.body
    // to escape the transformed ancestors that would otherwise capture its
    // `position: fixed`, which also takes it outside the page's theme scope.
    // Without the class every --lol-* variable resolves to nothing and the card
    // renders as floating transparent text.
    <div
